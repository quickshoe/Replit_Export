...
- **Agent Usage Expansion:** Agent Usage is no longer expanded separately. The `expandSingleElement` method now handles all expansions uniformly during the line-by-line walk, including Agent Usage. The `aria-expanded="true"` guard prevents accidentally collapsing already-expanded sections. The separate `expandAgentUsageInElement` method is retained but no longer called from walkAndExtract.
- **Charge Detail Extraction:** Scoped to only the Agent Usage section (parent element of the "Agent Usage" heading), not the full event container. Uses exact "Agent Usage" text match for heading detection. Relaxed filters (children.length 10, flexible dollar regex, inline "Label $X.XX" detection). Single extraction pass after all expansions. Diagnostic debug logging for troubleshooting.
...
- **Output Generation:** Exports data into multiple formats:
    ...
    - **CSV:** `all-events.csv` (combined messages, checkpoints, work entries), `chat.csv` (clean chat messages only), `work-tracking.csv` (structured work data with index number, description from charge detail labels, and dedup by index), `work-summary.csv` (daily aggregated totals with human-readable duration and numeric minutes column), and `agent-usage-details.csv` (individual charge line items).
