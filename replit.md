...
**Core Functionality:**
- **Browser Automation:** Playwright supports a two-phase headed/headless launch strategy. By default, extraction runs in headless mode (no visible browser) for speed. If login is needed, the browser relaunches in headed mode so the user can log in manually, then switches back to headless after login completes. When `--verbose` is used, the browser stays headed throughout so the user can watch all DOM operations live. Session cookies are saved for persistence.
