...
- **Agent Usage Expansion:** Simplified to directly click the "Agent Usage" text element. Finds the most specific element whose text is exactly "Agent Usage" (or "Agent Usage $X.XX") and clicks it. Falls back to clicking parent if the text element itself isn't clickable.
- **Work Metrics Preservation:** Work metrics (actions, lines, code changed, usage fee) are captured from the first extraction before Agent Usage expansion. After AU expansion, only charge details are updated from the re-extraction, ensuring work metrics are never lost.
- **Charge Detail Extraction:** Searches the full event container (not just EndOfRunSummary) for charge details after Agent Usage expansion. Uses exact "Agent Usage" text match for heading detection. Relaxed filters (children.length 10, flexible dollar regex, inline "Label $X.XX" detection). Diagnostic debug logging for troubleshooting.
