...
- **Agent Usage Expansion:** Simplified to directly click the "Agent Usage" text element. Finds the most specific element whose text is exactly "Agent Usage" (or "Agent Usage $X.XX") and clicks it. No parent/grandparent fallbacks to avoid accidentally collapsing sections. If the text element isn't clickable, expansion is skipped.
- **Charge Detail Extraction:** Scoped to only the Agent Usage section (parent element of the "Agent Usage" heading), not the full event container. Uses exact "Agent Usage" text match for heading detection. Relaxed filters (children.length 10, flexible dollar regex, inline "Label $X.XX" detection). No post-expansion re-extraction - charge details come from the initial extraction pass only. Diagnostic debug logging for troubleshooting.
